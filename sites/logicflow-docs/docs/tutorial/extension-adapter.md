...
- 对于一个节点，我们需要知道这个节点的 **id**、[类型](tutorial/extension-adapter#类型)、**位置**、**文本**、[properties](tutorial/extension-adapter#properties)
- 对于一个边，我们则需要知道这个边的 **id**、[类型](tutorial/extension-adapter#类型)、起始节点 id（**sourceNodeId**）、目标节点 id（**targetNodeId**）、**文本**、[properties](tutorial/extension-adapter#properties) 以及边的起点位置（**startPoint**），边的终点位置（**endPoint**）。
